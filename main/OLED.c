...
esp_err_t init_i2c_driver(void){
...
esp_err_t update_display_values(){
	...
	clear_oled();
		...
		get_data_JSON(tmpObjArray);
		for (int i = 0 ; i < cJSON_GetArraySize(tmpObjArray); i++){  
            tmpObj = cJSON_GetArrayItem(tmpObjArray, i);
            temp = (float)(cJSON_GetObjectItem(tmpObj,"temp")->valuedouble);
			id = (cJSON_GetObjectItem(tmpObj,"id")->valueint);
			sprintf(text, "Sensor%d: %.2f", id, temp);
